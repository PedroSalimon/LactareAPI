package com.lactare.api.exceptions.handler;

import com.lactare.api.exceptions.DatabaseException;
import com.lactare.api.exceptions.ResourceNotFoundException;
import com.lactare.api.exceptions.dto.CustomErrorDTO;
import com.lactare.api.exceptions.dto.ValidationErrorDTO;
...
public class GlobalExceptionHandler {
    ...
    // 400 - JSON malformado / corpo invalido (nao da para desserializar)
    ...
    public ResponseEntity<CustomErrorDTO> handleHttpMessageNotReadable(HttpMessageNotReadableException e,
        ...
        CustomErrorDTO err = new CustomErrorDTO(Instant.now(), status.value(),
                "Requisicao invalida (JSON malformado ou corpo nao interpretavel).",
                request.getRequestURI());
        return ResponseEntity.status(status).body(err);
    }

    // 400 - tipo invalido em PathVariable/RequestParam (ex.: /usuarios/abc quando espera Long)
    @ExceptionHandler(MethodArgumentTypeMismatchException.class)
    public ResponseEntity<CustomErrorDTO> handleTypeMismatch(MethodArgumentNotValidException e,
                                                             HttpServletRequest request){
        HttpStatus status = HttpStatus.BAD_REQUEST; //400
        CustomErrorDTO err = new CustomErrorDTO(Instant.now(), status.value(),
                "Requisicao invalida (parametro com tipo/formato incorreto).",
                request.getRequestURI());
        return ResponseEntity.status(status).body(err);
    }
